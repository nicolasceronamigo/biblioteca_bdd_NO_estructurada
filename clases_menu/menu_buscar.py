...
class MenuBuscar(Menu):
    def buscar_valoracion_minima(self):
        try:
            ...
            documentos = list(self.coleccion.find({"valoracion": {"$gte": valoracion}}, {"_id": -1, "codigo_libro": 1, "libro.titulo": 1, "valoracion": 1}))
    ...
    def buscar_antes_anno(self):
        try:
            ...
            documentos = list(self.coleccion.find({"libro.anio_publicacion": {"$lte": anno}}, {"_id": -1, "codigo_libro": 1, "libro.titulo": 1, "libro.annio_publicacion": 1}))
            mostrar_elementos(documentos)
        except:
    ...
    def buscar_coincidencia_nombre_copia(self):
        ...
        documentos = list(self.coleccion.find({"libro.titulo": {"$regex": nombre, "$options": "i"}}, {"_id": -1, "codigo_libro": 1, "libro.titulo": 1}))
        mostrar_elementos(documentos)
    ...
    def buscar_rango_fecha_prestamo(self):
        ...
        fecha_final = input("Ingrese hasta que fecha (inclusive) quiere buscar en formato YYYY-MM-DD: ")
        try:
            date_fecha_inicial = datetime.strptime(fecha_inicial, "%Y-%m-%d")
            date_fecha_final = datetime.strptime(fecha_final, "%Y-%m-%d")
            documentos = list(self.coleccion.find({"prestamos": {"$elemMatch": {"fecha_prestamo": {"$gte": date_fecha_inicial, "$lte": date_fecha_final}}}}, {"_id": -1, "codigo_libro": 1, "libro.titulo": 1, "prestamos.fecha_prestamo": 1}))
            mostrar_elementos(documentos)
        except:
    ...
    def buscar_copias_usuario(self):
        ...
        documentos = list(self.coleccion.find({"prestamos.usuario.nombre": nombre_usuario}, {"_id": -1, "codigo_libro": 1, "libro.titulo": 1, "prestamos.usuario.nombre": 1}))
        mostrar_elementos(documentos)